...
double double_integr(double x1, double x2, int nx, double y1, double y2, int ny, double (*func)(double, double))
{
    ...
    double sum = 0.0, x, y;
    y = y1;
    for(int i = 0; i<ny; y += dy,i++)
    {
        x = x1;
        for(int j = 0; j<nx; x += dx,j++)
            sum += func(x, y);
    }
    return dx * dy * sum;
}
...
double integr3D(double x1, double x2, int nx, double y1, double y2, int ny, double z1, double z2, int nz, double (*func)(double, double, double))
{
    ...
    z = z1;
    for(int i = 0; i < nz; z += dz,i++)
    {
        y = y1;
        for(int j = 0; j < ny; y += dy,j++)
        {
            x = x1;
            for(int k = 0; k < nx; x += dx,k++)
                sum += func(x, y, z);
        }
    }
    return dx * dy * dz * sum;
...
int main(void)
{
    ...
    {
    case 1: // 6.1
        if (TEST) printf("Zad. 6.1 - Całka jednowymiarowa (wsk. do funkcji, met. prostokąt lewostr.)\n");
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków [x1 x2 n]: ");
        ...
        break;
    case 2: // 6.2
        if (TEST) printf("Zad. 6.2 - Całki jednowymiarowe (wsk. do funkcji, met. prostokąt, trapez)\n");
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków [x1 x2 n]: ");
        scanf("%lf %lf %d", &x1, &x2, &n);
        ...
        for(int j=0; j < 4; j++)
        {
            ...
            {
                ic = (*intf[i])(x1, x2, n, objf[j]); //wskazanie funkcji całkującej z parametrami zawierającymi wskazanie funkcji do całkowania
        ...
        break;
    case 3: // 6.3
        if (TEST) printf("Zad. 6.3 - Całka podwójna (wsk. do funkcji)\n");
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków x [x1 x2 nx]: ");
        scanf("%lf %lf %d", &x1, &x2, &nx);
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków y [y1 y2 ny]: ");
        ...
        break;
    case 4: // 6.4
        if (TEST) printf("Zad. 6.4 - Całka potrójna (wsk. do funkcji)\n");
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków x [x1 x2 nx]: ");
        scanf("%lf %lf %d", &x1, &x2, &nx);
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków y [y1 y2 ny]: ");
        scanf("%lf %lf %d", &y1, &y2, &ny);
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków z [z1 z2 nz]: ");
        ...
        break;
    case 5: // 6.5
        if (TEST) printf("Zad. 6.5 - Całka podwójna (struktury -> prosta budowa całki wielokrotnej)\n");
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków x [x1 x2 nx]: ");
        scanf("%lf %lf %d", &x1, &x2, &nx);
        if (TEST) printf("Wpisz zakres całkowania i liczba kroków y [y1 y2 ny]: ");
        scanf("%lf %lf %d", &y1, &y2, &ny);
